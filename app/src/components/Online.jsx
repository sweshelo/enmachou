...
const Online = () => {
  const { isLoading, onlinePlayer, currentStage } = useSelector((state) => state.recordsReducer)
    ...
    <div id="ranking-wrapper">
      <div className="ranking">
        ...
        <p className="description">{
          isLoading
            ? `読み込み中です…`
            : <span>{`現在 ${onlinePlayer?.length}人 がオンラインと推定されます`}<br />{`現在時刻 ${new Date().toLocaleTimeString('ja-JP')}は 『${currentStage}』です`}</span>
          }</p>
        {
